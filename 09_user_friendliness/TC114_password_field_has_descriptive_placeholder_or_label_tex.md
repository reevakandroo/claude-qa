# TC114: Password field has descriptive placeholder or label text

| Field | Detail |
|-------|--------|
| **Test ID** | TC114 |
| **Module** | User Friendliness |
| **Type** | ✅ Positive |
| **Status** | ⬜ Pending |

## Preconditions

- [**PC001**](../00_preconditions/PC001_app_loaded_at_login_url.md) — App is open at the login page; no user is logged in

## Execution Steps

| Step | Action |
|------|--------|
| 1 | Observe the password input field |
| 2 | Check for a placeholder or label |

## Expected Results

Password field shows helpful text such as "Enter your password".

---
*Module: User Friendliness · Type: Positive*
